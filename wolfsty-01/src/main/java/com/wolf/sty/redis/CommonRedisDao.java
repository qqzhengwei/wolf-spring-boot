...
import com.wolf.sty.service.redis.IRedisDataSource;
 ...
 * redis公共处理
 */
@Repository("commonRedisDao")
public class CommonRedisDao {

    private static final Logger log = LoggerFactory.getLogger(CommonRedisDao.class);
    ...
    public List mget(List<byte[]> keys) {
        ShardedJedis shardedJedis = redisDataSource.getRedisClient();
        if (shardedJedis != null) {
            boolean broken = false;
            try {
