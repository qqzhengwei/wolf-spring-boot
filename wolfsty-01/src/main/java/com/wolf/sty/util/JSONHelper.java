...
* @version 创建时间：2016年11月3日 上午10:00:13
*
 */
public final class JSONHelper {
	...
	public static void main(String[] args) {
		...
		myMap.put("name", "张三");
		List<TabMenu> list = new ArrayList<TabMenu>();
		TabMenu obj1 = new  TabMenu();
		obj1.setIcon("icon1");
		obj1.setIconColor("黄色");
		list.add(obj1);
		TabMenu obj2 = new  TabMenu();
		obj2.setIcon("icon2");
		obj2.setIconColor("黄色2");
		list.add(obj2);
		myMap.put("list", list);
		System.err.println(collection2json(myMap));;*/
	}
	 ...
	 * @return
	 */
	public static <T, D> List<T> toBeanList(String jsonString, Class<T> mainClass,
                                            String detailName, Class<D> detailClass) {
		List<T> returnList = FastList.newInstance();
		JSONArray jsonArray = JSONArray.fromObject(jsonString);
		Iterator<Object> it = jsonArray.iterator();
        while (it.hasNext()) {
        	JSONObject jsonObject =  JSONObject.fromObject(it.next());
    		JSONArray jsonArrayD = (JSONArray) jsonObject.get(detailName);
    		T mainEntity = JSONHelper.toBean(jsonObject, mainClass);
    		List<D> detailList = JSONHelper.toList(jsonArrayD, detailClass);
    		try {
    			BeanUtils.setProperty(mainEntity, detailName, detailList);
    			returnList.add(mainEntity);
    		} catch (Exception ex) {
	...
	public static <T, D> T toBean(String jsonString, Class<T> mainClass,
		...
		if(jsonObject.containsKey("data")){
			if(UtilValidate.isEmpty(jsonObject.get("data"))){
				jsonObject.put("data",FastList.newInstance());
			}
		}
